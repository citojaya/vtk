...
# parse command line
p = OptionParser(usage="""usage: %prog [options] <particle_smallest_diameter> <par_largest_diameter><drum_diameter><drum_length> <outputfile>

Construct parameters.h and generate sample.dump
A box of ordered particles are generated within the mill geometry
...
if len(args) != 5:
   p.print_help()
   sys.exit(1)
(par_smallest_diameter,par_largest_diameter, drum_diameter,drum_length,out_file) = args
...
# Generate particles within a box which has boundaries defined by
# zmin, zmax and drum_diameter
x_steps = int(drum_diameter/par_largest_diameter)
y_steps = x_steps
z_steps = int((zmax-zmin)/par_largest_diameter)

print dx,dy,dz

for k in range(z_steps+1):
  temp_z = zmin + par_largest_diameter*k
  for j in range(y_steps+1):
    temp_y = -0.5*drum_diameter + par_largest_diameter*j
    for i in range(x_steps+1):
      temp_x = -0.5*drum_diameter + par_largest_diameter*i
      if(math.sqrt(temp_x*temp_x+temp_y*temp_y) < (0.5*drum_diameter-0.25*par_largest_diameter)):
        ...
        count += 1
no_of_particles = count

fout.write("SIMULATION TIME 0.0\n")
fout.write("TIME\n")
fout.write("#\n")
fout.write("+ 0.0\n")
fout.write("#\n")
fout.write("np "+str(count)+"\n")
